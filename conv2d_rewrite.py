...
def conv2d_rewrite(input, filter,strides,padding):
    ...
    if padding == 'SAME':
         ...
         output_size = [int(np.ceil((img_row)/strides)),int(np.ceil((img_col)/strides)),out_depth]
...
#below for testing
input = tf.random.truncated_normal((1,5,5,1), stddev=0.1, dtype=tf.dtypes.float64)
filter = tf.random.truncated_normal((3,3,1,1), stddev=0.1, dtype=tf.dtypes.float64)

test=input
test= conv2d_rewrite(input,filter.numpy(),2,'SAME')
#print(test)
t_zero = []

tf_test=tf.nn.conv2d(input,filter, strides=[1, 2, 2, 1], padding='SAME')
if tf_test.shape == test.shape:
    t_zero=tf_test-test
    print(t_zero,'zeros')
print(test[0,2,:,0])
print(type(test))
